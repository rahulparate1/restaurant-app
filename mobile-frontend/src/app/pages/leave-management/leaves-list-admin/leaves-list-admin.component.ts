...
@Component({
  selector: "app-leaves-list-admin",
  templateUrl: "./leaves-list-admin.component.html",
  styleUrls: ["./leaves-list-admin.component.css"],
})
export class LeavesListAdminComponent {
  designation: string = "";
  user: any;
  id: any;
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: "Leaves" },
      { label: "Employee Leaves List", active: true },
    ];
    ...
    swalWithBootstrapButtons
      ...
      .then(async (result) => {
        if (result.value) {
          this.apiService.startLoader();
          // Call the delete API
          (await this.service.deleteLeave(id)).subscribe({
            next: (res) => {
              this.apiService.stopLoader();
              swalWithBootstrapButtons.fire(
                "Deleted!",
                "Leave and balances have been updated.",
                "success"
              );
              this.getLeaveList(); // Refresh the leave list
            },
            error: (error) => {
              this.apiService.stopLoader();
              swalWithBootstrapButtons.fire(
                "Error!",
                "Unable to delete leave. Please try again.",
                "error"
              );
            },
          });
  ...
  openaprovel(id: any) {
    // Navigate to the LeaveRejectComponent with the leave ID
    this.router.navigate(["leave-management/leave-approve/" + id]);
  }

  // Another method for rejecting leave (you can define this similarly)
  goToReject(id: any) {
    // Navigate to the leave reject route, replace with your desired route if needed
    this.router.navigate(["leave-management/leave-reject/" + id]);
  }
